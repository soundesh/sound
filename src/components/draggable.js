...
function Draggable() {
  ...
  const initialValue = ["These","Words", "Can", "Be", "Dragged",  "And", "Dropped","Over", "On",
  ...
  function onDocumentMouseMove(mouseMoveEvent) {
    ...
    for (var i = 0; i < droppables.length; i++) {
      if (droppables[i].contains(point) && !droppables[i].isBeingDraggedOver) {
        ...
        !droppables[i].contains(point) &&
        droppables[i].isBeingDraggedOver
      ) {
  ...
  window.onload = function () {
    ...
    var availableMetricsDroppable = new Droppable(availableMetrics, function (
      draggable
    ) {
    ...
    var selectedMetricsDroppable = new Droppable("next_metric", function (
      draggable
    ) {
